...
def rotate_rect(scr, x, y, a, b, alf, color):
    pygame.draw.polygon(scr, color, [(x, y), (x + a * cos(alf), y + a * sin(alf)),
                                     (x + a * cos(alf) - b * sin(alf),
                                      y + a * sin(alf) + b * cos(alf)),
...
class Gun:
    ...
    def fire2_end(self, event):
        ...
        new_ball.r += 5
        self.an = atan2((event.pos[1] - new_ball.y),
    ...
    def draw(self):
        ...
        rotate_rect(self.screen, self.gunpos, HEIGHT -
                    50, 15, 100, -pi / 2 + self.an, GREY)
        rotate_rect(self.screen, self.gunpos, HEIGHT - 50, 15,
                    self.f2_power, -pi / 2 + self.an, self.color)
        pygame.draw.rect(self.screen, GREY,
                         (self.gunpos - 50, HEIGHT - 65, 100, 50))
    ...
    def draw_lives(self):
        for i in range(self.lives):
            pygame.draw.circle(
                self.screen, BLOOD, (WIDTH / 2 + 20 * (2 * i - self.lives / 2), 575), 15)
            f1 = pygame.font.Font(None, 50)
            text1 = f1.render('Hp', True, (180, 0, 0))
            screen.blit(text1, (WIDTH / 2 + 30 *
                        (2 * i - self.lives / 2), 575 - 40))
    ...
    for bomb in bombs:
        for b in balls:
            if b.bombhittest(bomb):
                bombs = bombs[:bombs.index(bomb)] + \
                    bombs[bombs.index(bomb) + 1:]
